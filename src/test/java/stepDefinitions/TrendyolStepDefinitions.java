...
public class TrendyolStepDefinitions {
    ...
    public void yardım_sayfasına_gidildiğini_doğrular() {
        homePage.yardimLinkWebElement.click();
        assertTrue(homePage.yardimSayfaLinkWebElement.isDisplayed());

    }

    @Then("Sıkça Sorulan Sorular sayfasına gidildiğini doğrular")
    public void sıkça_sorulan_sorular_sayfasına_gidildiğini_doğrular() {
        homePage.sikcaSorularLinkWebElement.click();
        //assertTrue(homePage.sikcaSorularSayfaLinkWebElement.isDisplayed());
    }

    @Then("Nasıl İade Edebilirim sayfasına gidildiğini doğrular")
    public void nasıl_i̇ade_edebilirim_sayfasına_gidildiğini_doğrular() {
        homePage.nasilIadeEdebilirimLinkWebElement.click();
        assertTrue(homePage.nasilIadeEdebilirimSayfaLinkWebElement.isDisplayed());
    }

    @Then("İşlem Rehberi sayfasına gidildiğini doğrular")
    public void i̇şlem_rehberi_sayfasına_gidildiğini_doğrular() {
        homePage.islemRehberiLinkWebElement.click();
        assertTrue(homePage.islemRehberiSayfaLinkWebElement.isDisplayed());

    }

    @Then("Trendyol Fırsatları sayfasına gidildiğini doğrular")
    public void trendyolFırsatlarıSayfasınaGidildiğiniDoğrular() {
        homePage.trendyolFirsatLinkWebElement.click();
        ReusableMethods.bekle(1);
        Set<String> tumHandleDegerleri = Driver.getDriver().getWindowHandles();
        String ikinciSayfaHandleDegeri = "";
        for (String handle : tumHandleDegerleri
        ...
        assertTrue(homePage.trendyolFirsatSayfaLinkWebElement.isDisplayed());

    }
//********************************************************************
    @Then("Apple store mobil uygulama linkinin sayfasina yonlendirilir")
    public void appleStoreMobilUygulamaLinkininSayfasinaYonlendirilir() {
        homePage.appStoreLinkElement.click();
        ReusableMethods.bekle(1);
        Set<String> tumHandleDegerleri = Driver.getDriver().getWindowHandles();
        String ikinciSayfaHandleDegeri = "";
        for (String handle : tumHandleDegerleri
        ...
        assertTrue(homePage.appleHomeLinkElement.isDisplayed());
